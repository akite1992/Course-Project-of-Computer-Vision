...
for epoch=start+1:opts.numEpochs
  ...
  val = opts.val ;
  if numGpus <= 1
    [net,stats.train] = process_epoch(1,opts, getBatch, epoch, train, learningRate, imdb, net) ;
    [~,stats.val] = process_epoch(0,opts, getBatch, epoch, val, 0, imdb, net) ;
  else
    spmd(numGpus)
      [net_, stats_train_] = process_epoch(1,opts, getBatch, epoch, train, learningRate, imdb, net) ;
      [~, stats_val_] = process_epoch(0,opts, getBatch, epoch, val, 0, imdb, net_) ;
    end
    net = net_{1} ;
  ...
  for i=1:numel(opts.errorLabels)
   fprintf(' %s:%.3g', opts.errorLabels{i}, info.train.error(i,epoch));
  end
  fprintf('\n');
  for i=1:numel(opts.errorLabels)
   fprintf(' %s:%.3g', opts.errorLabels{i}, info.val.error(i,epoch));
  end
  fprintf('\n');
end
...
% -------------------------------------------------------------------------
function  [net_cpu,stats,prof] = process_epoch(ifaug, opts, getBatch, epoch, subset, learningRate, imdb, net_cpu)
% -------------------------------------------------------------------------
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    [im, labels] = getBatch(imdb, batch,ifaug) ;
